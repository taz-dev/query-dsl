...
public class QuerydslBasicTest {
    ...
    public void aggregation() {
        //단일 타입이 아닐 경우 Tuple을 씀(실무에서는 DTO를 뽑아 씀)
    ...
    public void theta_join() {
        ...
        List<Member> result = queryFactory
                .select(member)
                .from(member, team) //from절에 여러 엔티티를 선택해서 세타조인(외부 조인 불가능)
    ...
    //서브쿼리
    //from절의 서브쿼리 해결방안(select절, where절은 서브쿼리 가능)
    ...
    @Test
    public void concat() {
        //member.age.stringValue()부분이 중요한데, 문자가 아닌 다른 타입들은
        //stringValue()로 문자로 변환할 수 있음 -> ENUM을 처리할 때 자주 사용
